...
The expressions in a context `select` must produce series that are all
...
The resulting dataframe, after applying aggregating expressions,
...
Because expressions are lazy, when you use an expression inside a
